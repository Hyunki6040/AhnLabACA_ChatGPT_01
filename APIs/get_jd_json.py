...
async def get_jd_json(request: jdRequest):
    ...
    response_schemas = [
        ...
        ResponseSchema(name="keyword", description="채용공고에 작성된 업계와 직무에 대해 유사하고 관련한 키워드들을 list 형태로 추출. 예를 들어 데이팅앱, 매칭, IT, 소셜 매칭, 플랫폼, 엔터, 크리에이터, 팬덤 플랫폼, 콘텐츠, 상품, 온오프라인 통합, 커뮤니티, 콘텐츠 기획제작, 크리에이터 B2B, 병의원, 치의과, 기공소,  B2B, 글로벌, B2B, 초기전략, 웹툰, 콘텐츠, 글로벌, 이커머스, 서비스 기획, Web, App, Admin, 동남아시아, 중동아시아, 여성유저, 심리상담, 콘텐츠, 병의원, 멘탈헬스케어, 디지털마케팅, google, meta, 심리상담, 콘텐츠 등과 같이 직무 및 업계에 관한 단어들"),
        ResponseSchema(name="process", description="채용공고에 작성된 지원 절차를 불렛포인트 형식으로 작성"),
        ResponseSchema(name="documents", description="채용공고에 작성된 제출 필요 서류를 불렛포인트 형식으로 작성"),
